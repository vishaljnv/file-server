...
NOTES:
* This is a TCP Pre-threaded File Server for Upload and Download of a file.
* Number of threads to create must be mentioned in command line option -c.

@USAGE:
    $ ./TCPPrethreadedFileServer -p port -c 4
     OR
    $ ./TCPPrethreadedFileServer -c 4
*/
...
void childMainLoop(int listeningSock);
...
int main(int argc, char *argv[]){
    ...
    while((ch = getopt(argc, argv, "c:p:")) != -1){  //Get command line options
        switch(ch){
    ...
    for(i = 0; i < numOfThreads; i++){
        //create threads
        if(pthread_create(&thread_id, &thread_attrs, (void * (*)(void *))childMainLoop, (void *)listeningSock) < 0){
            perror("create thread failed!");
        }
    }

    while(1); //in main process; wait forever
}

void childMainLoop(int listeningSock){
    /*Accept a connection, service the client and repeat*/
    int connectedSock;
