...
NOTES:
* This is a TCP Multiprocess File Server for Upload and Download of a file, with one process per client.

@USAGE:
    $ ./TCPMultiprocessFileServer -p port
     OR
    $ ./TCPMultiprocessFileServer
*/
...
int main(int argc, char *argv[]){
    ...
    while(1){
        ...
        if((pid = fork()) != 0){ //parent here
            close(connectedSock); //close connected socket
        }
        else{                    //child here
            close(listeningSock);  //close listening socket
            exit(serviceClient(connectedSock));  //service the client
        }
    }
}
