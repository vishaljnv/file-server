...
NOTES:
* This is a TCP Client for file download from the RPC File Server.
...
int main(int argc, char *argv[]){
    ...
    if(argc != 3){ //exit if command line arguments are missing
    ...
    printf("connecting...\n");
    if((handle = connectToFileServer(host)) == NULL){ //establish a connection to the TCP file server
...
CLIENT *connectToFileServer(char *host){
    ...
    handle = clnt_create(host, FILESERVER, FILESERVERVERS, "tcp"); //Create client handle
    if(handle == NULL){
        return NULL;
    }
...
int storeFile(char *file_name, int file_size, char *data){
    /*Store the file data in a local file*/
