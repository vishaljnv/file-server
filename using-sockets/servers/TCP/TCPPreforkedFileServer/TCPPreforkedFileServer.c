...
NOTES:
* This is a TCP Pre-forked File Server for Upload and Download of a file. 
* Number of children to fork must be mentioned in command line option -c.

@USAGE:
    $ ./TCPPreforkedFileServer -p port -c 5
     OR
    $ ./TCPPreforkedFileServer -c 5
*/
...
int main(int argc, char *argv[]){
    ...
    for(i = 0; i < numOfChildren; i++){
        ...
        if(pid == 0){ //This is child
            while(1){
                ...
                serviceClient(connectedSock);  //service the client
                close(connectedSock);
            }
        }
        else if(pid > 0){
            //parent here! Just sit back and do nothing.
        }
        else{
            perror("fork failed!");
        }
    ...
    while(wait(NULL));  //wait for all the children to finish, in case they exit!

    close(listeningSock);  //closing listening Socket; unused
}
